...
function CollBox(a, b) {
	var c = this.container = document.documentElement;//getElementById(a);
	var b = this.opts = b || {};
	this.width = window.innerWidth//b.width || c.offsetWidth;
	this.height = window.innerHeight//b.height || c.offsetHeight;
	this.child = []
}
...
CollBox.prototype.ballRun = function() {
	...
	this.ballRunTimer = setInterval(function() {
		...
		for (var i = 0; i < c; i++) {
			var d = a[i].opts;
			var b = d.b;
			var scrollTop = g.container.scrollTop;

			if (d.x < b) {
				d.x = b;
				d.sx *= -1
			}
			var yb = b + (scrollTop > 0? scrollTop: 0)
			if (d.y < yb) {
				d.y = yb;
				d.sy *= -1
			}
			...
			var f = h - b + (scrollTop > 0? scrollTop: 0)
